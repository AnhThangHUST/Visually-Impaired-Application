...
{
	cv::Mat small, page_mask, src_mask, img_src2;
	std::vector<cv::Point> page_outline;
	std::vector<ContourInfo> contours_out;
	std::vector<std::vector<ContourInfo>> spans, spans2;
	std::vector<std::vector<cv::Point2d>> span_points;
	...
	cv::Mat texture_ = cv::Mat::zeros(small.size(), small.type());
	std::vector<cv::Point> point2;
	for (int i = 0; i < line_point.size(); i++)
		point2.push_back(cv::Point((int)line_point[i].x, (int)line_point[i].y));
	cv::Point tmp;
	tmp = point2[2];
	point2[2] = point2[3];
	point2[3] = tmp;
	...
	cv::fillConvexPoly(texture_,							//Image to be drawn on
										 ...
										 0);
	cv::bitwise_and(texture_, small, out2);
	// cv::namedWindow("ConvexPoly", cv::WINDOW_NORMAL);
	// //	cv::resize(out2, out2, cv::Size(), 0.25, 0.25);
	// cv::imshow("ConvexPoly", out2);
	// cv::waitKey(0);
	small = out2.clone();
	cv::resize(texture_, src_mask, img_src.size());
	cv::bitwise_and(img_src, src_mask, img_src2);
	// cv::namedWindow("ConvexPoly", cv::WINDOW_NORMAL);
	// //	cv::resize(out2, out2, cv::Size(), 0.25, 0.25);
	// cv::imshow("ConvexPoly", img_src2);
	//cv::waitKey(0);
